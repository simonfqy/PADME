...
def main():
  ...
  def dataset_input_fn(loaded_data, testing, perform_shuffle=False, num_epoch=1):
    '''
    def decode_csv(line):
      parsed_line = tf.decode_csv(line, [[0.] for j in range(FEATURE_DIM+1)])
      label = parsed_line[-1:]
      del parsed_line[-1]
    ...
    base_skip = sum(train_obs[:(fold_index[0]-1)]) + sum(test_obs[:
      (fold_index[0]-1)])
    extra_skip = 0
    total_take = train_obs[(fold_index[0]-1)]
    if testing:
      extra_skip = train_obs[(fold_index[0]-1)]
      total_take = test_obs[(fold_index[0]-1)]
    total_skip = base_skip + extra_skip

    #print("total_take: {:d} \n".format(total_take))
    #print("total_skip: {:d} \n".format(total_skip))

    #dataset = (tf.contrib.data.TextLineDataset(file_path).skip(total_skip)
    #  .take(total_take).map(decode_csv, num_threads = 8, output_buffer_size=512))
  ...
  def train_input_fn():
    fold_index[0] += 1
    return dataset_input_fn(loaded_data, False, perform_shuffle=True, num_epoch = 8)
