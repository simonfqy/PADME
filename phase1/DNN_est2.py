...
INPUT_PATH = "SimBoost/data/davis_cv.csv"
LOG_DIR = "logs2/"
PATIENCE = 3
...
def main():
  ...
  def my_model(features, labels, mode):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      net = tf.layers.dropout(net, rate=0.05, training=True)
    else:
    ...
    for layer_id, num_hidden_units in enumerate([32, 32, 32]):
      with tf.variable_scope('hiddenlayer_%d' % layer_id, 
        ...
        net = tf.layers.dense(net, units=num_hidden_units, use_bias=True)        
